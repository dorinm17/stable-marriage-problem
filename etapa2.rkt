...
(define (get-pref-list pref person)
  (cdar (filter (λ (row) (equal? (car row) person)) pref)))
...
(define (get-partner engagements person)
  (define matching-engagement
    (find-first (λ (e) (equal? (car e) person))
                engagements))
  
  (if matching-engagement
      (cdr matching-engagement)
      #f))
...
(define (update-engagements engagements p1 p2)
  (change-first (λ (e) (equal? (car e) p1)) engagements (cons p1 p2)))
...
(define (better-match-exists? p1 p2 p1-list pref2 engagements)
  (and (not (null? engagements))
       (or (and (not (equal? (caar engagements) p2))
                (preferable? p1-list (caar engagements) p2)
                (preferable? (get-pref-list pref2 (caar engagements)) p1 (cdar engagements)))
            (better-match-exists? p1 p2 p1-list pref2 (cdr engagements)))))
...
(define (stable-match? engagements mpref wpref)
  (define (stable-match-iter iterator)
    (cond [(null? iterator) #t]
          [(better-match-exists? (cdar iterator)
                                 (caar iterator)
                                 (get-pref-list mpref (cdar iterator))
                                 wpref
                                 engagements) #f]
          [else (stable-match-iter (cdr iterator))]))

  (stable-match-iter engagements))
