...
(define (match person engagements pref1 pref2 queue)
  (let loop ([p-list (get-pref-list pref1 person)])
    (if (null? p-list)
        (cons (cons #f person) engagements)
        (let* ([candidate (car p-list)]
               [partner (get-partner engagements candidate)]
               [c-list (get-pref-list pref2 candidate)])
          (cond [(member candidate queue) (loop (cdr p-list))]
                [(not partner) (update-engagements engagements candidate person)]
                [(preferable? c-list person partner) (match partner (update-engagements engagements candidate person) pref1 pref2 queue)]
...
(define (path-to-stability engagements mpref wpref queue)
  (define (rev_eng eng) (map (λ (e) (cons (cdr e) (car e))) eng))

  (cond [(null? queue) engagements]
        [(member (car queue) (get-men mpref)) (path-to-stability (match (car queue) engagements mpref wpref (cdr queue))
                                                                 mpref
                                                                 wpref
                                                                 (cdr queue))]
        [else (path-to-stability (rev_eng (match (car queue) (rev_eng engagements) wpref mpref (cdr queue)))
                                 mpref
                                 wpref
                                 (cdr queue))]))
...
(define (update-stable-match engagements mpref wpref)
  (define (remove-list lst1 lst2) (filter (λ (elem) (not (member elem lst2))) lst1))
  
  (let* ([unstable (get-unstable-couples engagements mpref wpref)]
         [queue (get-couple-members unstable)]
         [room-engagements (remove-list engagements unstable)])
...
(define (build-stable-matches-stream pref-stream)
  (if (stream-empty? pref-stream)
      empty-stream
      (let* ([prefs (stream-first pref-stream)]
             [mpref (car prefs)]
             [wpref (cdr prefs)]
             [engagements (gale-shapley mpref wpref)])
        (let loop ([s (stream-rest pref-stream)]
                   [e engagements])
          (stream-cons e (if (stream-empty? s)
                             empty-stream
                             (loop (stream-rest s) (update-stable-match e 
                                                                        (car (stream-first s))
                                                                        (cdr (stream-first s))))))))))
